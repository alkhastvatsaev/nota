...
[Settings → Payment methods → Apple Pay](https://dashboard.stripe.com/settings/payment_methods/apple_pay) → **Add domain** → `app.heynota.app` → **Verify**.
...
curl https://api.stripe.com/v1/payment_method_domains \
  -u "$STRIPE_SECRET_KEY:" \
  -d domain_name=app.heynota.app
```

Tester en **Safari** sur Mac avec une carte dans Wallet. Vérifier : `curl -I https://app.heynota.app/.well-known/apple-developer-merchantid-domain-association`
